...
export default function HomeContent(props: any) {
    ...
    const storeDocuments = async() => { 
        await fetch('/api/storeDocuments').then((res) => {
            if (res.ok) {
                setEmbedsLoaded(true);
            } else {
                alert({ title: 'Error', message: 'An error occurred while fetching the documents.' });
            }
        }
        )
    }
    ...
    return (
        ...
        <div className="flex flex-col pt-12 h-max space-y-32">
            {embedsLoaded === false && (<PreLoad/>)}
            {embedsLoaded === true && chatActive === false && (
                <>
